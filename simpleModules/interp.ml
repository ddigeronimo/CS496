...
and
  add_module_definition (en:env) (AModBody vdefs):env =
  snd @@ List.fold_left (fun (env,renv) (AValDef(var,decl))  ->
      let ev=eval_expr env decl
          in (ExtendEnv(var,ev,env),ExtendEnv(var,ev,renv))) (en,EmptyEnv)
    vdefs
and
  add_module_definitions (en:env) (ms:mdecl list):env =
  List.fold_left (fun curr_en (AMod(mname,minterface,mbody)) ->
      ExtendEnvMod(mname,add_module_definition curr_en mbody,curr_en) ) en
    ms
 and
  eval_prog (Ast.AProg(ms,e)) = eval_expr (add_module_definitions init_env ms) e
