...
and
  ...
  | RefType of texpr
and
  vdecl = AValDecl of string*texpr
and
 vdef = AValDef of string*expr

type interface = ASimpleInterface of vdecl list
type module_body = AModBody of vdef list
type mdecl = AMod of string*interface*module_body
type prog = AProg of (mdecl list)*expr
