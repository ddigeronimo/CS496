...
and
  ...
  | App(e1,e2)     -> apply_proc (eval_expr en e1) (List.map (eval_expr en) e2)
  | For(x, e1, e2, e3) ->
    let v1 = eval_expr en e1 in
    let v2 = eval_expr en e2 in
    for i = (numVal_to_num v1) to (numVal_to_num v2) do (eval_expr (extend_env en x (NumVal i))e3) done; UnitVal
